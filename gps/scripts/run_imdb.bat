...
echo Running Adversarial Detection Experiments with Multiple Word Selection Strategies - IMDB Dataset
...
REM Set fixed TOP_N value for IMDB dataset
set TOP_N=25
...
FOR %%M IN (roberta deberta) DO (
    set MODEL_NAME=%%M_imdb
    REM Base directory, attack name will be appended
    set BASE_DATA_DIR=data/imdb/%%M
    ...
    echo.
    echo All IMDB experiments completed for model variant %%M with TOP_N: %TOP_N%!
)

echo.
echo All IMDB experiments completed for all model variants!
endlocal 