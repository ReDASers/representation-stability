...
echo Running Adversarial Detection Experiments with Multiple Word Selection Strategies - AG News Dataset
...
REM Set fixed TOP_N value for AG News dataset
set TOP_N=25
...
FOR %%M IN (roberta deberta) DO (
    set MODEL_NAME=%%M_ag_news
    REM Base directory, attack name will be appended
    set BASE_DATA_DIR=data/ag_news/%%M
    ...
    echo.
    echo All AG News experiments completed for model variant %%M with TOP_N: %TOP_N%!
)

echo.
echo All AG News experiments completed for all model variants!
endlocal
